...
public abstract class YellowPageLoader {
    private static final String TAG = "YellowPageLoader";
    ...
    //书本对象
    protected YellowCollBookBean mCollBook;
    //监听器
    protected OnPageChangeListener mPageChangeListener;

    //页面显示类
    private YellowPageView mPageView;
    //当前显示的页
    ...
    public void saveRecord() {
        ...
        mBookRecord.setBookId(mCollBook.getYellowId());
...
//            currentPage = Integer.parseInt(collBook.getStartpage());
        ...
        mBookRecord = BookRecordHelper.getsInstance()
                .findBookRecordById(mCollBook.getYellowId());
        if (mBookRecord == null) {
    ...
    void drawContent(Bitmap bitmap) {
        ...
        if (mPageMode == YellowPageView.PAGE_MODE_SCROLL) {
            canvas.drawColor(mPageBg);
        }
        /******绘制内容****/
        if (mStatus != STATUS_FINISH) {
